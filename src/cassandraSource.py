...
class LocalCassandraSource:
    ...
    def join(self, left_table_name, right_table_name, left_column, right_column):
        data_records = self.session.execute("SELECT * FROM {}.{};".format(self.keyspace, left_table_name))._current_rows
...
class CassandraSource:
    ...
    def join_both_sorted(self, left_table_name, right_table_name, left_column, right_column):
        data_devices = self.session.execute("SELECT * FROM {}.{};".format(self.keyspace,right_table_name))._current_rows
        records_single = self.session.execute("SELECT * FROM {}.{};".format(self.keyspace,left_table_name))._current_rows
        i = 0
        j = 0
        pom = 0
    ...
    def max(self, table_name, column_name, group_by):
        sql_query = "SELECT {}, MAX({}) FROM {}.{} GROUP BY {};".format(group_by, column_name, self.keyspace, table_name, group_by)
        rslt = self.session.execute(sql_query)
        return rslt._current_rows

    def min(self, table_name, column_name, group_by):
        sql_query = "SELECT {}, MIN({}) FROM {}.{} GROUP BY {};".format(group_by, column_name, self.keyspace, table_name, group_by)
        rslt = self.session.execute(sql_query)
        return rslt._current_rows

    def avg(self, table_name, column_name, group_by):
        sql_query = "SELECT {}, AVG({}) FROM {}.{} GROUP BY {};".format(group_by, column_name, self.keyspace, table_name, group_by)
        rslt = self.session.execute(sql_query)
        return rslt._current_rows

    def sum(self, table_name, column_name, group_by):
        sql_query = "SELECT {}, SUM({}) FROM {}.{} GROUP BY {};".format(group_by, column_name, self.keyspace, table_name, group_by)
        rslt = self.session.execute(sql_query)
        return rslt._current_rows